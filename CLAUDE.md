...
ROS 2 Humble workspace with three hello-world packages implementing the same publisher node pattern. The hello-world nodes publish `std_msgs/String` messages on the `chatter` topic at 1 Hz.
...
The ROS 2 MCP server (LCAS ros2_mcp) will return as an appendix lesson with working setup instructions; its `.mcp.json` was removed until then.
